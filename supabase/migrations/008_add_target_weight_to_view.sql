-- Update participant_rankings view to include target_weight
...
SELECT
  ...
  -- Goal progress percentage (null if no target set)
  CASE
    WHEN p.target_weight IS NOT NULL AND p.initial_weight > p.target_weight THEN
      ROUND((
        (p.initial_weight - COALESCE(lw.latest_weight, p.initial_weight))
        / (p.initial_weight - p.target_weight) * 100
      )::numeric, 1)
    ELSE NULL
